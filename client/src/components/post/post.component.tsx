    ...
    <>
      <Grid item xs={12} md={6} lg={4}>
        <Card sx={{ maxWidth: 345, overflow: 'visible' }}>
          ...
          <CardActions>
            <Box display="flex" justifyContent="space-between" width="100%" sx={{ px: '0.5rem' }}>
              <Box display="flex" alignItems="center">
                <Avatar alt="cart image" src={`${SERVER_ENDPOINT}/api/static/users/${post.user.photo}`} />
                <Typography
                  variant="body2"
                  sx={{
                    ml: '1rem'
              ...
              <div className="post-settings">
                ...
                <ul className="menu">
                  <li onClick={() => setOpenPostModal(true)}>
                    <ModeEditOutlineOutlinedIcon fontSize="small" sx={{ mr: '0.6rem' }} />
                    Edit
                  </li>
                  <li onClick={() => onDeleteHandler(post.id)}>
                    <DeleteOutlinedIcon fontSize="small" sx={{ mr: '0.6rem' }} />
                    Delete
                  </li>
                </ul>
              </div>
      ...
      <PostModal openPostModal={openPostModal} setOpenPostModal={setOpenPostModal}>
