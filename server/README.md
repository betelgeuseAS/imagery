...
- **Hot Reloading**: [Concurrently](https://github.com/open-cli-tools/concurrently) Hot reloading with concurrently
...
Docker is an operating system-level virtualisation platform that allows software to be packaged, deployed, and managed separately from the host operating system. It creates a standardised environment, called a container, in which applications can run independently of the host system configuration, simplifying the deployment, scaling, and management of applications.
See the [Docker](https://eslint.org/) for more information.
...
See the [Jest](https://jestjs.io/) for more information.
...
Automate codebase style and formatting checks using a linter to ensure consistent and readable code. Rectify issues by running the linter and applying fixes.
...
Run `npm run start` so you can compile Typescript(.ts) files in watch mode
...
Run `npm test` to make sure all Typescript tests pass.
